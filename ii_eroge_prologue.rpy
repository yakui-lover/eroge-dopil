...
init 410:
    ...
    python:
        text_images = []
        def show_slowtext(text, n, padding=None, style = style.thoughts_day):
            if padding==None:
                padding = 20*n
...
##    jump d2_evening_map_prep 
    ...
    menu:
        ...
        "День 2":
            ...
            "Выберите пропущенные решения"
            menu:
                "Поздороваться со Славей":
                    $ sp_crg = sp_crg + 1
                    $ lp_sl = lp_sl + 1
                "Проигнорировать":
                    pass
            menu:
                "Сбежать от Двачевской":
                    $ d1_tricked = True
                    $ d1_ug_help = True
    ...
    $ clear_slowtext()
    $ show_slowtext("Немногие знакомые и родственники, считавшие своё мнение очень важным, не упускали момента поучать меня.", -5)
    $ show_slowtext("«Перед тобой открыты все дороги», — говорили они.", -3)
    $ show_slowtext("«Все ушли вперёд, а ты не сделал ни шагу с места», — говорили они.", -1)
    $ show_slowtext("И хотя я делал вид, что меня это не сильно заботит, в глубине души я чувствовал себя уязвлённым.", 1)
    $ show_slowtext("Тогда я не мог припомнить ничего из своего скудного жизненного опыта, за что себя хвалить, за что гордиться собой.", 3)
    $ show_slowtext("До какого-то времени это было правдой, но однажды всё изменилось.", 5)
    $ clear_slowtext()

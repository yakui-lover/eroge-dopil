...
label ii_eroge_prologue:
    ...
    "Автобус приехал. Несколько необычный, старый, какие чаще ходят в провинциальных городах, но тускло освещённая табличка «410» чуть выше лобового стекла развеяла сомнения. Мой маршрут."
    stop sound_loop
    window hide
    $ renpy.pause(2)
    scene anim intro_10
    ...
    window show
    "По ступенькам я поднялся навстречу свету, льющемуся из салона автобуса, и двери захлопнулись за моей спиной." 

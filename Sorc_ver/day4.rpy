...
label day_4_start:
    ...
    #play music music_no_tresspassing channel 7
    play music music_just_think channel 7
    ...
    $renpy.music.set_volume (0.7, channel=7)
    #play music music_everyday_theme channel 7
    play music music_smooth_machine channel 7
    ...
    "Видимо, О.Д. поняла мое состояние, потому что сказала:"
    ...
    "Грусть притупилась, но вместо нее возник стыд за то, что ударил О.Д. и в очередной раз проспал все утро."
    ...
    "И еще у меня было странное предчувствие, потому что О.Д. принесла с собой большой пакет."
    if day_3_badmorning == 1:
        "Даже больше, чем вчера."
    else:
        pass
    "О.Д. заметила, куда я смотрю, и снова улыбнулась."
    mt "Да-да! Это тебе."
    ...
    with flash
    show mt normal
    "Голос из... из сна - так это была О.Д!"
    ...
    with dissolve
    "Пока мы шли, О.Д. объясняла, что я должен буду сделать."
    ...
    if day_3_badmorning == 1:
        ...
        pass
    "Когда мы вышли к умывальникам, О.Д. заметно повеселела."
    ...
    "Вечером он врет М-тян о том, что сделал работу, и та несет ее своему начальству. Потому что эти книги были самыми настоящими и потому что ошибки не были выявлены - М-тян наказывают. И пятый день начинается, как и задумывалось, с того, что М-тян занята исправлением книг. (Если же герой исправляет ошибки, то М-тян уезжает. Здесь событийная вилка.)"
    $ set_mode_adv()
    "......................."
    return
...
label day_4_event_square:
    $ random = renpy.random.choice([1,4,3])
    if random == 1:
        scene bg ext_square_day
        "~Странный памятник.~"
    else:
        if random == 4:
            scene bg ext_square4_day
...
label day_4_eventAf_library:
    $ random = renpy.random.choice([1,4,4])
    if random == 1:
        scene bg ext_library_day
        "БГ вечерний."
        "Закрыто."
